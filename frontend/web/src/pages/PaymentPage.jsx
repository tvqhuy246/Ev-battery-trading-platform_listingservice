...
function PaymentPage() {
  ...
  const handlePayWithWallet = async () => {
    ...
    if (walletBalance < order.price) {
      alert(
        `Số dư ví không đủ để thanh toán. Cần ${order.price.toLocaleString(
          'vi-VN'
    ...
    try {
      ...
      const msg = err.response?.data?.error || err.message || 'Không thể thanh toán bằng ví';
      alert(msg);
    } finally {
        ...
        <div className="payment-grid">
          ...
          <div className="payment-order-info">
            ...
            <div className="card" style={{ marginTop: '1.5rem' }}>
              <h2 className="card-title">Thanh toán bằng ví nội bộ</h2>
              {walletLoading ? (
                <p>Đang tải số dư ví...</p>
              ) : walletError ? (
                <p className="payment-error-text">Lỗi khi tải số dư ví: {walletError}</p>
              ) : (
                <>
                  <p>
                    Số dư ví hiện tại:{' '}
                  ...
                  <p>
                    ...
                    <strong>{order?.price?.toLocaleString('vi-VN')} đ</strong>
                  </p>
                  {walletBalance < (order?.price || 0) && (
                    <p className="payment-error-text">
                      Số dư không đủ. Vui lòng nạp thêm tiền vào ví trước khi thanh toán.
                    </p>
                  )}
                  {walletPaid && (
                    <p className="payment-success-notice-text">
                      ✅ Đã thanh toán bằng ví thành công. Đang chuyển hướng...
                    </p>
                  )}
                  <button
                  ...
                  >
                    {payingWithWallet ? 'Đang thanh toán...' : '💳 Thanh toán bằng ví'}
                  </button>
                </>
              )}
            </div>

            {/* Component theo dõi trạng thái thanh toán (giữ lại để tương thích Casso nếu cần) */}
